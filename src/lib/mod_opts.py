...
class opts(object):
  def __init__(self):
    ...
    self.parser.add_argument('--vis_thresh', type=float, default=0,
                             help='visualization threshold.') ## changed from 0.3 to 0
    ...
    # vehint
    self.parser.add_argument('--not_hp_bound_reg_offset', action='store_true',
                             help='not regress local offset for vehicle internal bounding box')
  ...
  def parse(self, args=''):
    ...
    opt.hp_bound_reg_offset = not opt.not_hp_bound_reg_offset
  ...
  def update_dataset_info_and_set_heads(self, opt, dataset):
    input_h, input_w = dataset.default_resolution     # 512, 512 for apollo dataset
    #opt.mean, opt.std = dataset.mean, dataset.std
    opt.num_classes = dataset.num_classes             # 1 for apollo dataset

    # input_h(w): opt.input_h overrides opt.input_res overrides dataset default
    input_h = opt.input_res if opt.input_res > 0 else input_h   # default input_res is -1
    ...
    if opt.task == 'exdet':
      ...
      if opt.reg_hp_offset:
        ...
        opt.heads = {'hm': opt.num_classes, 'wh': 2, 'hps': 12, 'hp_bound_wh': 2}
        if opt.reg_offset:
            opt.heads.update({'reg': 2})
            opt.heads.update({'hp_bound_reg': 2})
        if opt.hm_hp:
