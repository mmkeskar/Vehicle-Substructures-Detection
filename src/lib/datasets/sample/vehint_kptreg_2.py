...
class VehIntKptRegDataset(data.Dataset):
    """
    This class uses the annotation file data-apollocar3d/annotations/24kptreg_train.json.
    This annotation file was created from the data-apollocar3d/annotations/apollo_train_24_modkeypoints.json
    file after finding the centers of the vehicle internals.

    The script fix_json_files.py was used to create the annotation files for the training, and validation
    sets.
    """
    ...
    def __getitem__(self, index):
        ...
        # for predicting the height and width of bounding box
        wh = np.zeros((self.max_objects, 2), dtype=np.float32)
        # for predicting offsets from the center of object to exact center point
        reg = np.zeros((self.max_objects, 2))
        reg_mask = np.zeros((self.max_objects), dtype=np.uint8)
        # mark which objects are present
        ...
        # the offset from the integer form of keypoint to keypoint
        ...
        for i in range(num_objs):
            ...
            if (top_corner or bottom_corner) and w*h > self.min_area:
                ...
                if len(ann['keypoints']) != 0:
                    ...
                    num_kps = 0
                    for kpt in all_kpts:
                        if rand_col <= kpt[0] < rand_col + input_res and rand_row <= kpt[1] < rand_row + input_res and kpt[2] == 2:
                    ...
                    for j in range(self.num_kpts):
                        kpt = kpts[5*j+4]
                        corners = all_kpts[5 * j:5 * j + 4, :]
                        num_vis = np.sum(corners[:, 2])
                        vis_corners = np.delete(corners, np.argwhere(corners[:, 2] != 2), 0)
                        if num_vis > 0:
                            if kpt[2] == 1:
                                kpt[:2] = max(0, )
